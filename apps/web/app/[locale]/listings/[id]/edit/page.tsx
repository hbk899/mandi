...
interface ListingDetail {
  ...
  price?: number; currency: string; priceType: 'fixed' | 'negotiable' | 'free' | 'contact'
...
export default function EditListingPage() {
  ...
  useEffect(() => {
    ...
    api.get<ListingDetail>(`/listings/${listingId}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(l => {
        setTitleEn(l.titleEn)
        setTitleUr(l.titleUr ?? '')
        setDescriptionEn(l.descriptionEn ?? '')
        setDescriptionUr(l.descriptionUr ?? '')
        setCategorySlug(l.category.slug)
        setCityId(l.city?.id ?? '')
        setLocationText(l.locationText ?? '')
        setPriceType(l.priceType)
        setPrice(l.price != null ? String(l.price) : '')
        setImages(l.images.map(img => ({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const body = {
        ...
        ...(titleUr.trim() ? { titleUr: titleUr.trim() } : { titleUr: null }),
        ...
        priceType,
        currency: 'PKR',
        ...((priceType === 'fixed' || priceType === 'negotiable') && price ? { price: Number(price) } : { price: null }),
      }
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-neutral-800 mb-6">
        {isUr ? 'اشتہار ترمیم کریں' : 'Edit Listing'}
      </h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={() => router.back()}
            className="flex-1 border border-neutral-200 text-neutral-600 py-3 rounded-xl font-semibold text-base transition-colors hover:bg-neutral-50"
          >
            {isUr ? 'منسوخ' : 'Cancel'}
          </button>
          <button
